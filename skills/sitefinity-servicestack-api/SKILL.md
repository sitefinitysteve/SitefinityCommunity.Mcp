...
**Global ServiceStack configuration is off-limits - anywhere.** The `AppHost` you are plugging into is the same one Sitefinity's own backend (the AdminApp, the page editor services, OData helpers, the MCP plugin) runs on, and ServiceStack's configuration is process-global: `JsConfig.*` (`DateHandler`, `IncludeNullValues`, `EmitCamelCaseNames`, `ExcludeTypeInfo`, ...), `JsConfig<T>` per-type overrides for platform types, `appHost.Config` / `SetConfig(new HostConfig { ... })`, global request/response filters, `ContentTypes` registrations, and `JsonSerializer` defaults. Setting any of them in `Global.asax`, a `Bootstrapper.Initialized` handler, or your plugin's `Register` changes how **every** Sitefinity service serializes, and the backend breaks in places far from your code (the classic case is `JsConfig.DateHandler = DateHandler.ISO8601`: the admin UI expects `/Date()/` and stops loading content). Scoped variants (`JsConfig.BeginScope()`, `JsConfig.With(...)`) are not a safe workaround either - they leak across the shared host's request pipeline. Keep every behavior change on **your own DTOs and services**: string properties for dates (below), initialized collections instead of `IncludeNullValues`, `[DataMember(Name = ...)]` for naming, and per-service `[AddHeader]` / response filters for content types. This is the single most expensive mistake on a Sitefinity site because it is invisible until an editor opens the backend.
