...
using Telerik.Sitefinity.Web.UI; // IndexRenderMode
...
// Custom attribute approach (ASP.NET Core renderer only)
[ExternalDataChoice]
[DataType(customDataType: KnownFieldTypes.Choices)]
public string DynamicChoiceSingle { get; set; }
```

> The custom attribute approach (`ExternalDataChoiceAttribute` + `ExternalChoicePropertyConfigurator`) is an **ASP.NET Core renderer only** extension point — not available in MVC. In MVC, use `[Choice(ServiceUrl = ...)]` directly. See [All Properties sample](https://github.com/Sitefinity/sitefinity-aspnetcore-mvc-samples/tree/master/src/all-properties).
