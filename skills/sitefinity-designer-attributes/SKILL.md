...
> **Companion skill**: this is the attribute *catalogue*. For building the widget around these properties - controller structure, routing and page titles, how the values actually persist (`PersistAsJson`), reading selections in the controller, views and script loading, toolbox icons - use `sitefinity-widget-expert`.
...
**Verified against Sitefinity 15.4 (build 15.4.8636; `Progress.Sitefinity.Renderer.dll` 15.4.8636.44): every attribute, property, constant and namespace below was checked against the decompiled Renderer and `Telerik.Sitefinity.dll` assemblies, and the "works in classic MVC" claims against a production MVC site that uses them.** Newer releases may add attributes; re-verify before relying on anything not listed.

**For MVC (.NET Framework 4.8)**:
```csharp
using Telerik.Sitefinity.Web.Services.PropertyEditor.Attributes;  // ConditionalVisibility (MVC subclass of the Renderer one)
...
using Telerik.Sitefinity.Modules.Pages.PropertyPersisters;    // PropertyPersistence (PersistAsJson, IsKey)
using Telerik.Sitefinity.Modules.GenericContent;              // DynamicLinksContainer
using Telerik.Sitefinity.Web.UI;                              // IndexRenderMode, ICustomWidgetVisualization
using Progress.Sitefinity.Renderer.Designers;                 // KnownFieldTypes, PropertyCategory
using Progress.Sitefinity.Renderer.Designers.Attributes;      // TableView, ContentSection, Choice, Content, MediaItem, KnownContentTypes, ...
using Progress.Sitefinity.Renderer.Entities;                  // NumericRange, DecimalNumericRange, FileTypes, ChoiceWithText
...
> **Important**: `Progress.Sitefinity.Renderer.dll` ships with Sitefinity 14.3+ — reference it from the project where your widget controllers live. `KnownFieldTypes` and `PropertyCategory` are classes with string constants — use them directly via `using Progress.Sitefinity.Renderer.Designers;`. Raw string values are in the [KnownFieldTypes Reference](#knownfieldtypes-reference).

### How these attributes reach the classic MVC designer (why they work in MVC at all)

The AdminApp page editor asks the server for widget metadata through `Telerik.Sitefinity.Web.Services.Contracts.Operations.Pages.PropertyEditor`. For an MVC widget, `ComponentAdaptorBase` calls `WidgetRendererResolver.GetPropertyConfigurator()`, which is the **Renderer assembly's `PropertiesConfigurator`** — the same engine the ASP.NET Core renderer uses. That engine:

- **Auto-serializes every attribute class marked `[DataContract]`** into `Meta_<AttributeName>_<Property>` entries the AdminApp reads. In 15.4 that is: `Choice`, `ConditionalVisibility`, `Content`, `TaxonomyContent` (serialized under the `Content` name), `ContentTypeSelector`, `DatePeriodSelector`, `DateSettings`, `DescriptionExtended` (as `Description`), `Dialog`, `DisplaySettings`, `FallbackToDefaultValueWhenEmpty` (as `EmptyValues`), `Group`, `LengthDependsOn`, `MediaItem`, `Mirror`, `Placeholder`, `RangeLimitation`, `Suffix`, `TableView`.
- **Handles explicitly**: `DisplayName`, `Description`, `Category`, `Browsable`, `ReadOnly`, `DefaultValue`, `DataType`, `ContentSection` (name + position), `SectionsOrder`, `EnumDisplayName`, `Icon`, `MappedType`, `Flags`, and the DataAnnotations validators (`Required`, `Range`, `StringLength`, `MinLength`, `MaxLength`, `RegularExpression`, `EmailAddress`, `Url`).
- **`IsNullable` and `InitialValue`** have no server-side consumer in 15.4 (a grep of the decompiled Renderer and Sitefinity assemblies finds only their definitions) — treat them as AdminApp-side or reserved; do not depend on them.
- **`ViewSelector`** is MVC-only: the classic `ViewSelectorPropertyConfigurator` in `Telerik.Sitefinity.dll` reads it.

So there is one attribute set for both renderers, minus a few attributes that only ship in the ASP.NET Core packages (flagged below as **Core renderer only**): `ColorPalette`, `Copy`, `ContentContainer`.

**Production-proven in a 15.4 MVC site** (counts of real widget properties using them): `ContentSection` (56), `ConditionalVisibility` (34), `Content` (16), `TableView` (7), `TaxonomyContent` (4), `Placeholder` (4), `ViewSelector` (3), `SectionsOrder` (2), `DescriptionExtended` (1), and `DataType` with `KnownFieldTypes.TextArea`, `CheckBox`, `RadioChoice`, `Html`, `Color`, `ChipChoice`, `DropdownWithText`. Everything else in this reference is verified to exist and to be serialized, but has not been exercised on that site.
...
| `bool` | Yes/No radio buttons | Use `[DataType(customDataType: KnownFieldTypes.CheckBox)]` for checkbox |
| `int` | Number input | |
...
When combined with `[DefaultValue]`, falls back to the default if the user clears the field. Optional `EmptyValues` string property lists which values count as "empty".
```csharp
...
### `[Mirror(fieldName, fieldProperty = null)]`
...
| `mirrorFieldProperty` | `string` | Which sub-property to mirror (for complex types); optional, defaults to `null` |

| Property | Type | Description |
|----------|------|-------------|
| `FieldName` | `string` | Source field name |
| `FieldProperty` | `string` | Source field sub-property |
| `EnableMirroring` | `bool` | Read-only, always `true` in 15.4 (there is no way to disable mirroring through the attribute) |
...
### `[Copy(Exclude = true)]` — **Core renderer only**
Excludes this property when the widget is duplicated/copied. **Not available in classic MVC**: no `CopyAttribute` exists in `Progress.Sitefinity.Renderer.dll` 15.4 or any other assembly in a 15.4 MVC site's `bin` (the only similarly named type is `Telerik.Sitefinity.SkipCopyAttribute` in `Telerik.Sitefinity.Model.dll`, an internal model attribute, not a designer attribute). It ships with the ASP.NET Core renderer packages.
```csharp
...
### `[ContentContainer]` — **Core renderer only**
Marks an HTML field as a content container (enables special content editing features). **Not available in classic MVC** — no `ContentContainerAttribute` exists in any 15.4 MVC-site assembly; it ships with the ASP.NET Core renderer packages.
```csharp
// ASP.NET Core renderer ONLY
...
Namespace `Telerik.Sitefinity.Modules.GenericContent` (`Telerik.Sitefinity.dll`); a marker attribute with no properties. Marks a property whose value contains Sitefinity dynamic links (`sfref`-style links resolved at render time) — consumed by the inline-editing and property services (`PageControlStrategy`, `WcfPropertyManager`); used with `LinkModel` inside complex objects.
```csharp
[DynamicLinksContainer]
...
> **Namespace (MVC)**: `Telerik.Sitefinity.Web.Services.Contracts.Operations.Pages.PropertyEditor.AttributeConfigurator.Attributes.ViewSelectorAttribute` in `Telerik.Sitefinity.dll` (NOT `Telerik.Sitefinity.Frontend`). Properties: `ComponentName`, `RegexFilter`. Handled by the MVC-only `ViewSelectorPropertyConfigurator`.
>
> **Limitation in classic MVC**: The `[ViewSelector]` attribute only discovers views in the **ResourcePackages theme folder** (`ResourcePackages/{Theme}/MVC/Views/{Widget}/`), NOT in the standard `Mvc/Views/` folder. If your custom widget views live in `Mvc/Views/`, the dropdown appears **empty**.
>
...
> **NOT AVAILABLE IN MVC (.NET Framework 4.8)** — verified against every `Telerik.Sitefinity*.dll` and `Progress.Sitefinity*.dll` in a 15.4 MVC site's `bin\` (type lists dumped with ilspycmd): `ColorPaletteAttribute` does not exist there. It ships only in the ASP.NET Core renderer packages (`Progress.Sitefinity.AspNetCore`). **Do not add it to MVC widgets** — it will not compile. The samples below are ASP.NET Core renderer only.
>
...
| `Enabled` | `bool` | `true` | Read-only (`{ get; } = true`) — cannot be set from the attribute |
...
- The edit view renders each sub-property using the standard Sitefinity field wrapper chain (see [Field Wrapper DOM Structure](#field-wrapper-dom-structure) below)
...
// Custom attribute approach (ASP.NET Core renderer only; ExternalDataChoice is YOUR attribute from the sample, not a platform type)
[ExternalDataChoice]
[DataType(customDataType: KnownFieldTypes.Choices)]
public string DynamicChoiceSingle { get; set; }
```

> The custom attribute approach (`ExternalDataChoiceAttribute` + `ExternalChoicePropertyConfigurator`, both defined in the sample project — no such types exist in the platform assemblies) is an **ASP.NET Core renderer only** extension point — not available in MVC. In MVC, use `[Choice(ServiceUrl = ...)]` directly. See [All Properties sample](https://github.com/Sitefinity/sitefinity-aspnetcore-mvc-samples/tree/master/src/all-properties).
...
| `Content` | `ContentContext[]` | Content context for each selected item (`Type` + `Variations[]`, each with `Source`, `Filter`, `DynamicFilterByParent`) |
| `ManualSelectionItems` | `ManualSelectionItem[]` | Manually selected items (`Index`, `Item`) |
| `CustomProperties` | `Dictionary<string, ContentCustomProperties>` | Per-item custom properties |

#### Declaring the Property

Pair with `[PropertyPersistence(PersistAsJson = true)]` so Sitefinity stores the selection as JSON. (Verified in `PageHelper.IsSupportedComplexProperty`: any class type defined in the `Progress.Sitefinity.Renderer` assembly — `MixedContentContext`, `LinkModel`, `NumericRange`, `FileTypes`, ... — is JSON-persisted automatically, so the attribute is technically redundant for them; keep it anyway for clarity. It is **required** for `IList<T>` / `List<T>` and for classes of your own.)
...
### Serializing LinkModel to JSON for a JS Frontend

When serializing link data to JSON for a JS frontend (see the companion skill `sitefinity-vue3-vite8-guide` for the config-island pattern), extract the properties you need:
...
| `allowMultipleSelecton` [sic] | `bool` | Allow selecting multiple items (the parameter name carries Progress's typo) |
| `standalone` | `bool` | Standalone mode (optional, default `false`) |

Properties: `ItemType`, `AllowMultipleSelection`, `Standalone`, `AllowRemove` (settable only as a named property, e.g. `[MediaItem("images", false, AllowRemove = true)]`).
...
### `[Icon("name")]`
Associates a Font Awesome icon with an **enum value** (name without `fa-` prefix). `AttributeUsage` is `AttributeTargets.Field` — it does not compile on a property.
...
Declares an initial value for a sub-property of a complex type. `AttributeUsage` is `AttributeTargets.Class` with `AllowMultiple = true` — it goes on the **complex type's class**, not on the property that uses it. No server-side consumer was found in the 15.4 Renderer or Sitefinity assemblies (definition only), so treat it as advisory.
...
### `[IsNullable(true)]`
Marks a property as nullable in the designer, allowing it to be cleared. `AllowMultiple = true`, property `Value`. No server-side consumer was found in the 15.4 assemblies (definition only) — nullable value types (`int?`, `DateTime?`) already render as clearable without it.
...
Configures range limitation behavior for numeric fields used with `KnownFieldTypes.RangeLimitation`. Properties `AllowZeroes` (bool) and `InitialRangeType` (enum `RangeLimitationRangeType`: `Between`, `MoreThan`, `LessThan`).

### `[ContentTypeSelector]`
Renders a content-type picker (use with `[DataType(customDataType: KnownFieldTypes.ContentTypeSelector)]`). Properties: `IsDynamicParentContentType`, `IsMultipleSelection`, `CanSelectAll`, `IsLifecycleContentType`, `IsInTable`, `UseFullTypeName` (all `bool`).

### `[DatePeriodSelector]`
Renders a date/time period picker (use with `[DataType(customDataType: KnownFieldTypes.DateTimePeriodSelector)]`). Properties: `DateTimeMode` (enum: `DateTime`, `DateOnly`, `TimeOnly`) and `FromToMode` (enum: `FromTo`, `FromOnly`, `ToOnly`).
...
| `KnownFieldTypes.DropdownWithText` | `"dropdownWithText"` | Dropdown with text input (pairs with the `ChoiceWithText` entity: `ChoiceValue` + `TextValue`) |
| `KnownFieldTypes.ContentTypeSelector` | `"contentTypeSelector"` | Content-type picker (pairs with `[ContentTypeSelector]`) |
| `KnownFieldTypes.DateTimePeriodSelector` | `"dateTimeSelector"` | Date/time period picker (pairs with `[DatePeriodSelector]`) |
