...
- **`allowRawSecrets`** — Optional (default false). Disables secret redaction for **logs only**. Ignored for prod-like environment names. Use only on local dev. (The config reader always redacts credentials regardless of this flag.)
