# SitefinityCommunity.Mcp — Codex Instructions

## What This Project Is

An MCP (Model Context Protocol) server that gives Codex direct access to Sitefinity CMS logs, diagnostics, and status. It runs as a .NET console app communicating via stdio, and exposes tools that Codex can call.
...
├── install-skills.ps1                 ← Interactive installer: Codex/Cursor/Codex/Copilot (project or global)
├── SitefinityCommunity.Mcp.slnx       ← Solution file
├── skills/                            ← Codex skills (installable via install-skills.ps1)
...
public sealed class MyNewTools
{
    ...
    [Description("What this tool does — Codex reads this to decide when to use it.")]
...
- Use `[Description]` — Codex uses this to decide when the resource is relevant
...
- **Plugin side** — `McpSecretRedactor.cs` (.NET 4.8, compiled regex). Wired into `McpFormsService` (field names + values) and `McpConfigService` (config values).
...
3. Verify with Codex — tools like `sitefinity_list_environments` don't need Sitefinity running
