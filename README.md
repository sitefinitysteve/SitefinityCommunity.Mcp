...
| `sitefinity-widget-expert` | Build and troubleshoot MVC widgets: controllers, routing and page titles, designer basics, persistence, toolbox icons |
...
| "Trace everything jane.doe@example.com did in the last 3 days" | search | `sitefinity_investigate_incident(query: "jane.doe@example.com", lookbackHours: 72)` |
...
3. **Search** — pass `query` without a `time`. Sweeps every source over `lookbackHours` for a case-insensitive plain substring — one user's full request trail (`query: "jane.doe@example.com"`), an order id, a URL path. The IIS log *is* scanned here; the line ceiling and time budget bound it.
